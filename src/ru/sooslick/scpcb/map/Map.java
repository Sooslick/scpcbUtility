...
public class Map {
    ...
    private void createMap(int seed) {
        int[][] mapTemp = new int[Map.MAP_WIDTH + 1][Map.MAP_HEIGHT + 1];
        ...
        String[][] mapName = new String[Map.MAP_WIDTH][Map.MAP_HEIGHT];

        int[] mapRoomID = new int[Map.ROOM4 + 1];

        x = Map.MAP_WIDTH / 2;
        y = Map.MAP_HEIGHT - 2;
        for (i = y; i <= Map.MAP_HEIGHT - 1; i++) {
            mapTemp[x][i] = 1;
        }
        ...
        do {
            ...
            for (i = 1; i <= yHallways; i++) {
                ...
                x2 = Math.max(2, Math.min(Map.MAP_WIDTH - 2, bbRand(x, x + width - 1)));
        ...
        for (y = 1; y <= Map.MAP_HEIGHT; y++) {
            zone = getZone(y);

            for (x = 1; x <= Map.MAP_WIDTH - 1; x++) {
                if (mapTemp[x][y] > 0) {
                    if (mapTemp[x][y] < 255)
        ...
        for (i = 0; i <= 2; i++) {
            ...
            if (temp > 0) {
                ...
                for (y = (Map.MAP_HEIGHT / zoneAmount) * (2 - i) + 1; y <= ((Map.MAP_HEIGHT / zoneAmount) * ((2 - i) + 1)) - 2; y++) {

                    for (x = 2; x <= Map.MAP_WIDTH - 2; x++) {
                        if (mapTemp[x][y] == 0) {
        ...
        for (i = 0; i <= 2; i++) {
            ...
            switch (i) {
                case 2:
                    zone = 2;
                    temp2 = Map.MAP_HEIGHT / 3;
                    break;
                case 1:
                    zone = Map.MAP_HEIGHT / 3 + 1;
                    temp2 = (int) (Map.MAP_HEIGHT * (2.0 / 3.0) - 1);
                    break;
                case 0:
                    zone = (int) (Map.MAP_HEIGHT * (2.0 / 3.0) + 1);
                    temp2 = Map.MAP_HEIGHT - 2;
            }
        ...
        int maxRooms = 55 * Map.MAP_WIDTH / 20;
        ...
        mapRoom = new String[Map.ROOM4 + 1][maxRooms];
        ...
        for (y = Map.MAP_HEIGHT - 1; y >= 1; y--) {
            ...
            for (x = 1; x <= Map.MAP_WIDTH - 2; x++) {
                temp = getConnections(mapTemp, x, y);
                if (mapTemp[x][y] == 255) {
                    int type = temp == 2 ? Map.ROOM2 : Map.ROOM1;
                    ...
                    switch (temp) {     // number of rooms in adjacement cells
                        case 1:
                            if (mapRoomID[Map.ROOM1] < maxRooms && mapName[x][y] == null) {
                                if (mapRoom[Map.ROOM1][mapRoomID[Map.ROOM1]] != null)
                                    mapName[x][y] = mapRoom[Map.ROOM1][mapRoomID[Map.ROOM1]];
                            }

                            r = createRoom(zone, Map.ROOM1, x * 8, y * 8, mapName[x][y]);
                            if (mapTemp[x][y + 1] > 0)
                                r.angle = 180;
                            else if (mapTemp[x - 1][y] > 0)
                                r.angle = 270;
                        ...
                        case 2:
                            if (getHorizontalConnections(mapTemp, x, y) == 2) {
                                if (mapRoomID[Map.ROOM2] < maxRooms && mapName[x][y] == null) {
                                    if (mapRoom[Map.ROOM2][mapRoomID[Map.ROOM2]] != null)
                                        mapName[x][y] = mapRoom[Map.ROOM2][mapRoomID[Map.ROOM2]];
                                }
                                r = createRoom(zone, Map.ROOM2, x * 8, y * 8, mapName[x][y]);
                                if (bbRand(1, 2) == 1) {
                                ...
                                if (mapRoomID[Map.ROOM2C] < maxRooms && mapName[x][y] == null) {
                                    if (mapRoom[Map.ROOM2C][mapRoomID[Map.ROOM2C]] != null)
                                        mapName[x][y] = mapRoom[Map.ROOM2C][mapRoomID[Map.ROOM2C]];
                                }

                                if (mapTemp[x - 1][y] > 0 && mapTemp[x][y + 1] > 0) {
                                    r = createRoom(zone, Map.ROOM2C, x * 8, y * 8, mapName[x][y]);
                                    r.angle = 180;
                            ...
                            break;
                        case 3:
                            if (mapRoomID[Map.ROOM3] < maxRooms && mapName[x][y] == null) {
                                if (mapRoom[Map.ROOM3][mapRoomID[Map.ROOM3]] != null)
                                    mapName[x][y] = mapRoom[Map.ROOM3][mapRoomID[Map.ROOM3]];
                            }

                            r = createRoom(zone, Map.ROOM3, x * 8, y * 8, mapName[x][y]);
                            if (mapTemp[x][y - 1] == 0)
                                r.angle = 180;
                            else if (mapTemp[x - 1][y] == 0)
                                r.angle = 90;
                            ...
                            break;
                        case 4:
                            if (mapRoomID[Map.ROOM4] < maxRooms && mapName[x][y] == null) {
                                if (mapRoom[Map.ROOM4][mapRoomID[Map.ROOM4]] != null)
                                    mapName[x][y] = mapRoom[Map.ROOM4][mapRoomID[Map.ROOM4]];
                            }

                            r = createRoom(zone, Map.ROOM4, x * 8, y * 8, mapName[x][y]);
                            mapRoomID[Map.ROOM4]++;
                            break;
                    }
                }
